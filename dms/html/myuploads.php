...
if(isset($_GET['delete'])){
  try {
    $storage->getBucket()->object($_GET['delete'])->delete();
    $ref_table=$_GET['date']."/".$_GET['q']."/".$_GET["c"];
    $reply=$database->getReference("Files")->removeChildren([$ref_table]);
    $deleted=true;
...
if(isset($deleted)){
  ...
  <span>The file '.$_GET["delete"].' has been deleted.</span>
  </div>
  </div>';
  unset($_GET["delete"]);
  unset($deleted);
 ...
 ?>
  ...
  <div class="card" id="tables">
    ...
    <div class="table-responsive text-nowrap">
      <table class="table" id="myUploads">
        <thead>
          <tr>
            <th hidden></th>
            <th>Control No.</th>
...
// $url="localhost:8080/dms2/dms/html/view.php?url=".$url;
      ...
      echo  "</td>
      <td><a href='".$_SESSION["env"]."dms/html/file.php?d=".$value['uploaddate']."&file=".$value['controlnumber']."&q=".$key1."'><strong>View Info</strong></a>
 | <a href='".$url."'><strong>Download</strong></a> | <a href='".$_SESSION["env"]."dms/html/myuploads.php?delete=".$value['filename']."&q=".$key1."&date=".$value['uploaddate']."&c=".$key2."'><strong>Delete</strong></a>
      </td>
        </tr>
      ";
    }
