...
if(isset($_GET["delete"])&&$_SESSION["isAdmin"]==1){
...
if (isset($_POST["feedbackupdate"])) {
  // $ref_table='Files/'.$_GET["d"]."/".$_GET['q']."/".$_GET["file"]."/Feedbacks/";

$controlnumber=$_GET['file'];
$dateuploaded=$_GET['d'];
$userid=$_GET['q'];
$msg=$_POST['feedbacks'];
  $data=[
  ...
  $ref_table='Files/'.$dateuploaded."/".$userid."/".$controlnumber."/Feedbacks/".$date;

  $updatequery_result=$database->getReference($ref_table)->set($data);



  // Files/2022-10-10/LA85oSgCfxbBg1WPBsODCo0qK7g2/20221010021019/Feedbacks/2022-11-10
  // echo $ref_table;
...
// echo 'Files/'.$_GET["d"]."/".$_GET['q']."/".$_GET["file"]."/Feedbacks/";
 // var_dump($fetchdata);
 // exit();
}

if(isset($_POST["actiontakensubmit"])){
  ...
  $userid=$_GET['q'];
    ...
    $ref_table='Files/'.$dateuploaded."/".$userid."/".$controlnumber;
    $updatequery_result=$database->getReference($ref_table)->update($data);
}
...
<?php
if (isset($_GET["file"])&&isset($_GET["d"])&&isset($_GET['q'])) {
  $fetchdata=$database->getReference('Files/'.$_GET["d"]."/".$_GET['q']."/".$_GET["file"])->getValue();


if ($fetchdata==null) {
echo '<div class="card">
  <h5 class="card-header">Records Not Found</h5>
</div>';
}else{
  echo ' <div class="card">
    <h5 class="card-header">File Information</h5>
    <!-- fileinformation -->
    <div class="col-md-12">
      <div class="card mb-4">
        <h5 class="card-header">
        <strong>Control #: '.$fetchdata['controlnumber'].'</strong> </br></br>
        <strong>Title: '.$fetchdata['title'].'</strong></h5>
        <div class="card-body">
        <div class="row">
        <div class="col-md-6">
        <div class="card shadow-none bg-transparent border border-primary">
        <div class="card-body">
        <p><strong>Uploaded by: </strong>'.$fetchdata['uploadedby'].'</p>
          <p><strong>Received by: </strong>'.$fetchdata['receivedby'].'</p>
          <p><strong>Routed to: </strong>'.$fetchdata['routedto'].'</p>
          <p><strong>Action Taken: </strong>';
          if(isset($fetchdata['actiontaken'])) echo $fetchdata['actiontaken']; echo '</p>
          <hr>
        </div>
        </div>
        </div>
        ...
        <div class="card shadow-none bg-transparent border border-primary">
        <div class="card-body">
        <p><strong>Upload Date: </strong>'.$fetchdata['uploaddate'].'</p>
        <p><strong>Agency: </strong>'.$fetchdata['agency'].'</p>
        <p><strong>Remarks: </strong>'.$fetchdata['remarks'].'</p>
        <p><strong>Filename: </strong>'.$fetchdata['actualfilename'].'</p>
...
<div class="card-body">
<div>
<form method="post" action="file.php?d='.$_GET['d'].'&file='.$_GET['file']."&q=".$_GET['q'].'">

  <label for="exampleFormControlTextarea1" class="form-label">Action Taken</label>
  <textarea class="form-control" name="actiontaken" id="exampleFormControlTextarea1" rows="3"></textarea>
<hr>
  <button type="submit" name="actiontakensubmit" class="btn btn-primary">Save changes</button>
          ...
          echo '<!-- Custom content with heading -->
          <div class="">
            <small class="text-light fw-semibold">FEEDBACKS</small>
            <div class="demo-inline-spacing mt-3">
              ...
              $dateuploaded=$_GET['d'];
              $userid=$_GET['q'];
              $feedbacktable='Files/'.$dateuploaded."/".$userid."/".$controlnumber."/Feedbacks/";
               $feedbackdata=$database->getReference($feedbacktable)->getValue();
               if($feedbackdata!=null){
                 krsort($feedbackdata);

foreach ($feedbackdata as $id => $rows) {
  echo '
    <a
    ...
    >
      <div class="d-flex justify-content-between w-100">
        <h6><i class="bx bxs-user-voice"></i> '.$rows['user'].'</h6>
        <small><i class="bx bxs-time"></i> '.$id.'</small>
      </div>
      <p class="mb-1"><i class="bx bx-message-dots"></i> &nbsp;
      '.$rows['msg'].'
      </p>
    </a>';
}
        ...
        echo'<!--/ Custom content with heading -->
          <form method="post" action="file.php?d='.$_GET['d'].'&file='.$_GET['file']."&q=".$_GET['q'].'">
          <div>
            <label for="exampleFormControlTextarea1" class="form-label">Feedbacks</label>
            <textarea class="form-control" name="feedbacks" id="exampleFormControlTextarea1" rows="3"></textarea>
          </div>
        <hr>

              <button type="submit" name="feedbackupdate" class="btn btn-primary">Save changes</button>
...
if ($_SESSION["isAdmin"]!=1) {
  echo "<a>";
        ...
        echo "<a data-bs-toggle='modal'
        data-bs-target='#assign' style='cursor: pointer;color: #6f71f1;'
        data-control='".$value['controlnumber']."'
        data-path='".$key."/".$key1."/".$value['controlnumber']."' data-assigned='";
if($value['routedto']==""){
echo "Unassigned";
}else{
echo $value['routedto'];
...
echo "'>";
      ...
      echo  "</td>
      <td>
      <a  href='".$_SESSION["env"]."/html/file.php?d=".$key."&file=".$value['controlnumber']."&q=".$key1."'><strong>View</strong></a> |
      <a  href='".$url."'><strong>Download</strong></a>";
if($_SESSION["isAdmin"]==1){
  echo "
  |<a  href='".$_SESSION["env"]."/html/file.php?d=".$key."&file=".$value['controlnumber']."&q=".$key1."&delete=true'><strong>Delete</strong></a>
  ";
}
      ...
      ";
      // <td  style='cursor: pointer'><input type='text' value='".$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']."?d=".$value['uploaddate']."&file=".$value['controlnumber']."' hidden/>view</td>
}
    }
}
  }
...
<div class="modal fade" id="assign" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
    <div class="modal-content">
      <form class="" action="javascript:void(0)" method="post" onsubmit="assign(this)">
      <div class="modal-header">
        <h5 class="modal-title" id="">Routed to</h5>
        <button
          type="button"
          class="btn-close"
          data-bs-dismiss="modal"
      ...
      <div class="modal-body">
        <div class="row g-2">
          <div class="col mb-0">
            <label for="remarks" class="form-label">Control Number:</label>
            <input type="text" id="ctrl" name="ctrl" class="form-control" placeholder="" disabled/>
          </div>
        </div>
        <div class="row g-2">
          <div class="col mb-0">
            <input type="text" name="path" id="path" value="" hidden>
            <label for="exampleFormControlSelect1" class="form-label">Routed to</label>
            <select class="form-select" name="routedto" id="exampleFormControlSelect1" aria-label="Routed to" required>
...
console.log('well');
    ...
    var ctrl = $(e.relatedTarget).data('control');
    var assigned = $(e.relatedTarget).data('assigned');

    //populate the textbox
    // console.log(path);
    $(e.currentTarget).find('select[name="routedto"]').val(assigned);
$(e.currentTarget).find('input[name="ctrl"]').val(ctrl);
